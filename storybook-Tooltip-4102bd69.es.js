import { r as R, R as Ft } from "./storybook-index-45401197.es.js";
function Lt(t) {
  return t.split("-")[1];
}
function Yt(t) {
...
const ke = function(t) {
  ...
  return t === void 0 && (t = {}), { name: "shift", options: t, async fn(e) {
    ...
    if (l) {
      ...
      T = Vt(T + h[m === "y" ? "top" : "left"], T, T - h[x]);
    }
    if (a) {
      const x = w === "y" ? "bottom" : "right";
      _ = Vt(_ + h[w === "y" ? "top" : "left"], _, _ - h[x]);
    }
...
function Ct(t, e, n, r) {
  ...
  if (s) {
    ...
    for (; d && r && x !== b; ) {
      const O = Tt(d), S = d.getBoundingClientRect(), E = getComputedStyle(d);
      S.x += (d.clientLeft + parseFloat(E.paddingLeft)) * O.x, S.y += (d.clientTop + parseFloat(E.paddingTop)) * O.y, m *= O.x, w *= O.y, T *= O.x, _ *= O.y, m += S.x, w += S.y, d = ot(d).frameElement;
    }
  }
...
const Le = { getClippingRect: function(t) {
  ...
  return { reference: Pe(e, await o(n), r), floating: { x: 0, y: 0, ...await l(n) } };
}, getClientRects: (t) => Array.from(t.getClientRects()), isRTL: (t) => at(t).direction === "rtl" }, ee = (t, e, n) => {
  const r = /* @__PURE__ */ new Map(), o = { platform: Le, ...n }, l = { ...o.platform, _c: r };
  return (async (a, s, f) => {
    ...
    let b = await w.getElementRects({ reference: a, floating: s, strategy: h }), { x, y: d } = Kt(b, p, _), O = p, S = {}, E = 0;
    for (let z = 0; z < T.length; z++) {
      ...
      x = G ?? x, d = q ?? d, S = { ...S, [M]: { ...S[M], ...J } }, E > 50 && console.warn(["Floating UI: The middleware lifecycle appears to be running in an", "infinite loop. This is usually caused by a `reset` continually", "being returned without a break condition."].join(" ")), k && E <= 50 && (E++, typeof k == "object" && (k.placement && (O = k.placement), k.rects && (b = k.rects === !0 ? await w.getElementRects({ reference: a, floating: s, strategy: h }) : k.rects), { x, y: d } = Kt(b, O, _)), z = -1);
    }
...
yt = ne, function() {
  ...
  function et(i, A, v, N, C) {
    ...
    for (L in v !== void 0 && (B = "" + v), function(g) {
      ...
      I.call(A, L) && !W.hasOwnProperty(L) && ($[L] = A[L]);
    if (i && i.defaultProps) {
...
} }, Fe = R.createContext({ getTooltipData: () => De });
function we(t = "DEFAULT_TOOLTIP_ID") {
  return R.useContext(Fe).getTooltipData(t);
}
const ie = async ({ elementReference: t = null, tooltipReference: e = null, tooltipArrowReference: n = null, place: r = "top", offset: o = 10, strategy: l = "absolute", middlewares: a = [je(Number(o)), ke(), Ne({ padding: 5 })] }) => {
  if (!t)
  ...
  return n ? (s.push({ name: "arrow", options: f = { element: n, padding: 5 }, async fn(p) {
    ...
    const d = ae(m), O = { x: w, y: T }, S = $t(_), E = Yt(S), z = await x.getDimensions(h), M = S === "y" ? "top" : "left", F = S === "y" ? "bottom" : "right", G = b.reference[E] + b.reference[S] - O[S] - b.floating[E], q = O[S] - b.reference[S], J = await (x.getOffsetParent == null ? void 0 : x.getOffsetParent(h));
    let k = J ? S === "y" ? J.clientHeight || 0 : J.clientWidth || 0 : 0;
    k === 0 && (k = b.floating[E]);
    const Y = G / 2 - q / 2, H = d[M], V = k - z[E] - d[F], I = k / 2 - z[E] / 2 + Y, W = Vt(H, I, V), et = Lt(_) != null && I != W && b.reference[E] / 2 - (I < H ? d[M] : d[F]) - z[E] / 2 < 0;
...
const le = ({ id: t, className: e, classNameArrow: n, variant: r = "dark", anchorId: o, anchorSelect: l, place: a = "top", offset: s = 10, events: f = ["hover"], positionStrategy: p = "absolute", middlewares: h, wrapper: m, children: w = null, delayShow: T = 0, delayHide: _ = 0, float: b = !1, noArrow: x = !1, clickable: d = !1, closeOnEsc: O = !1, style: S, position: E, afterShow: z, afterHide: M, content: F, html: G, isOpen: q, setIsOpen: J, activeAnchor: k, setActiveAnchor: Y }) => {
  const H = R.useRef(null), V = R.useRef(null), I = R.useRef(null), W = R.useRef(null), [et, rt] = R.useState({}), [ct, nt] = R.useState({}), [Q, ut] = R.useState(!1), [ft, pt] = R.useState(!1), vt = R.useRef(!1), wt = R.useRef(null), { anchorRefs: bt, setActiveAnchor: Et } = we(t), xt = R.useRef(!1), [i, A] = R.useState([]), v = R.useRef(!1);
  ...
  R.useEffect(() => {
    ...
    return d && ((y = H.current) === null || y === void 0 || y.addEventListener("mouseenter", lt), (P = H.current) === null || P === void 0 || P.addEventListener("mouseleave", dt)), K.forEach(({ event: it, listener: At }) => {
      D.forEach((Ot) => {
        var kt;
      ...
      var it, At;
      f.find((Ot) => Ot === "click") && window.removeEventListener("click", j), O && window.removeEventListener("keydown", g), d && ((it = H.current) === null || it === void 0 || it.removeEventListener("mouseenter", lt), (At = H.current) === null || At === void 0 || At.removeEventListener("mouseleave", dt)), K.forEach(({ event: Ot, listener: kt }) => {
        D.forEach((_e) => {
          var Ut;
          (Ut = _e.current) === null || Ut === void 0 || Ut.removeEventListener(Ot, kt);
        });
      });
    };
    ...
    E ? B(E) : b ? wt.current && B(wt.current) : ie({ place: a, offset: s, elementReference: k, tooltipReference: H.current, tooltipArrowReference: V.current, strategy: p, middlewares: h }).then((y) => {
  ...
  return ft ? Rt.exports.jsxs(m, { id: t, role: "tooltip", className: re("react-tooltip", St.tooltip, St[r], e, { [St.show]: X, [St.fixed]: p === "fixed", [St.clickable]: d }), style: { ...S, ...et }, ref: H, children: [G && Rt.exports.jsx($e, { content: G }) || F || w, Rt.exports.jsx(m, { className: re("react-tooltip-arrow", St.arrow, n, { [St["no-arrow"]]: x }), style: ct, ref: V })] }) : null;
}, Ie = ({ id: t, anchorId: e, anchorSelect: n, content: r, html: o, className: l, classNameArrow: a, variant: s = "dark", place: f = "top", offset: p = 10, wrapper: h = "div", children: m = null, events: w = ["hover"], positionStrategy: T = "absolute", middlewares: _, delayShow: b = 0, delayHide: x = 0, float: d = !1, noArrow: O = !1, clickable: S = !1, closeOnEsc: E = !1, style: z, position: M, isOpen: F, setIsOpen: G, afterShow: q, afterHide: J }) => {
  const [k, Y] = R.useState(r), [H, V] = R.useState(o), [I, W] = R.useState(f), [et, rt] = R.useState(s), [ct, nt] = R.useState(p), [Q, ut] = R.useState(b), [ft, pt] = R.useState(x), [vt, wt] = R.useState(d), [bt, Et] = R.useState(h), [xt, i] = R.useState(w), [A, v] = R.useState(T), [N, C] = R.useState(null), { anchorRefs: L, activeAnchor: $ } = we(t), B = (j) => j == null ? void 0 : j.getAttributeNames().reduce((g, u) => {
    var c;
  ...
  const tt = { id: t, anchorId: e, anchorSelect: n, className: l, classNameArrow: a, content: k, html: H, place: I, variant: et, offset: ct, wrapper: bt, events: xt, positionStrategy: A, middlewares: _, delayShow: Q, delayHide: ft, float: vt, noArrow: O, clickable: S, closeOnEsc: E, style: z, position: M, isOpen: F, setIsOpen: G, afterShow: q, afterHide: J, activeAnchor: N, setActiveAnchor: (j) => C(j) };
...
const be = (t) => t, xe = (t) => t, Se = ({
  ...
  return /* @__PURE__ */ Ft.createElement("span", { className: "cove-tooltip", style: a, ...s }, /* @__PURE__ */ Ft.createElement(
    "a",
    {
      id: h,
      role: "dialog",
