...
import { a as _ } from "./storybook-Icon-48ed169f.es.js";
...
const f = [
  ...
  const [e, c] = o.useState(!1), [n, d] = o.useState(!1), [l, v] = o.useState(null), [s, h] = o.useState(!1);
  o.useState(!1);
  const [b, x] = o.useState(null), m = o.useRef(null);
  o.useRef(null);
  const E = o.useRef(null), k = o.useCallback(
    ...
    const p = g === "MacOS" ? i.metaKey : i.altKey;
    if (p) {
      let u = i.key;
      isNaN(u) || u <= f.length && (i.preventDefault(), k(f[u - 1]));
    }
    p || (i.code === "KeyG" && d((u) => !u), i.code === "KeyR" && w());
  };
  o.useEffect(() => (document.addEventListener("keydown", N), () => document.removeEventListener("keydown", N)), []);
  const w = o.useCallback(() => {
    if (!s && m.current) {
      v(null), h(!0), d(!1), m.current.style.transition = "transform 800ms cubic-bezier(0.16, 1, 0.3, 1)", m.current.style.transform = "rotate(-360deg)";
      const i = setTimeout(() => {
        h(!1), m.current.style.transition = null, m.current.style.transform = "rotate(0deg)", m.current.style.transform = null;
      }, 400);
  ...
  return o.useEffect(() => d(!!l), [l]), o.useEffect(() => {
  ...
  ), f.map((i, p) => /* @__PURE__ */ a.createElement("button", { className: `cove-editor-utils__breakpoints-item${l === i ? " active" : ""}`, onClick: () => k(i), key: p }, i, "px")), /* @__PURE__ */ a.createElement(
    "button",
    {
      className: "cove-editor-utils__breakpoints-item",
      onClick: () => {
    ...
    /* @__PURE__ */ a.createElement("div", { className: "cove-editor-utils__breakpoints-reset", ref: m }, /* @__PURE__ */ a.createElement(_, { display: "rotateLeft" }))
  ))));
};
const D = (t) => {
