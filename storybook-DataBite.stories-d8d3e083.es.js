...
import { A as $e, a as re, b as le, c as oe, d as ce, D as Ze } from "./storybook-DataTransform-1390427f.es.js";
import { a as fe, S as Je } from "./storybook-Icon-b09f76c0.es.js";
...
import { L as ne, a as Ke } from "./storybook-coveUpdateWorker-364bbc36.es.js";
import { T as et, H as ge } from "./storybook-index-e5bf02db.es.js";
import { c as tt } from "./storybook-chroma-b564050a.es.js";
import { a as at, g as nt } from "./storybook-isSolr-cb863e7a.es.js";
...
function it(t, l, m) {
  var h = this, u = p.useRef(null), T = p.useRef(0), y = p.useRef(null), N = p.useRef([]), F = p.useRef(), c = p.useRef(), a = p.useRef(t), _ = p.useRef(!0);
  a.current = t;
  ...
  var S = p.useMemo(function() {
    var A = function(D) {
      ...
      if (!_.current)
        return !1;
      var I = D - u.current, U = D - T.current;
      ...
      if (G(D))
        return W(D);
      if (_.current) {
        var I = D - u.current, U = D - T.current, r = l - I, o = P ? Math.min(r, M - U) : r;
        X(k, o);
      }
    }, J = function() {
      ...
      if (N.current = D, F.current = h, u.current = U, r) {
        if (!y.current && _.current)
          return T.current = u.current, X(k, l), j ? A(u.current) : c.current;
        if (P)
...
function ut(t, l, m) {
  var h = m && m.equalityFn || st, u = mt(t), T = u[0], y = u[1], N = it(p.useCallback(function(c) {
    return y(c);
  }, [y]), l, m), F = p.useRef(t);
  return h(F.current, t) || (N(t), F.current = t), [T, N];
}
const $ = p.memo(({ label: t, section: l = null, subsection: m = null, fieldName: h, updateField: u, value: T, tooltip: y, type: N = "input", i: F = null, min: c = null, max: a = null, ..._ }) => {
...
), /* @__PURE__ */ e.createElement("span", { className: "edit-label column-heading" }, t), /* @__PURE__ */ e.createElement("span", { className: "cove-icon" }, T))), ae = p.memo(({ label: t, value: l, options: m, fieldName: h, section: u = null, subsection: T = null, required: y = !1, updateField: N, initial: F, ...c }) => {
  let a = "";
  if (Array.isArray(m))
  ...
  var k, J, D, I, U;
  const { config: t, updateConfig: l, loading: m, data: h, setParentConfig: u, isDashboard: T, isEditor: y } = p.useContext(Re), [N, F] = p.useState(!0), c = Ye(t, l, !0);
  p.useEffect(() => {
    if (u) {
      const r = _();
      delete r.newViz, u(r);
  ...
  const u = {
    ...
    innerRing: {
      fill: tt(ve[l].primary).darken(0.5)
    },
    text: {
      fill: "#FFF"
    }
...
const gt = (t) => {
  const { configUrl: l, config: m, isDashboard: h = !1, isEditor: u = !1, setConfig: T, link: y } = t, N = {
    ...
    coveLoadedHasRan: !1,
    container: null
  }, [F, c] = p.useReducer(pt, N), { config: a, loading: _, currentViewport: R, coveLoadedHasRan: j, container: V } = F, { title: P, dataColumn: M, dataFunction: S, imageData: A, biteBody: X, biteFontSize: G, dataFormat: W, biteStyle: k, filters: J, subtext: D } = a, { innerContainerClasses: I, contentClasses: U } = ot(a), r = new Ze(), o = new at((s) => {
    for (let f of s) {
      let z = nt(f.contentRect.width * 2);
