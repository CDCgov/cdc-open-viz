import { r as T, R as Se } from "./storybook-index-BmAYD2Ot.es.js";
function Ct(t) {
  return t.split("-")[1];
}
function zt(t) {
...
function Qt(t) {
  const e = ot(t);
  let r = Gt(t);
  for (; r && Le(r) && at(r).position === "static"; )
    r = Gt(r);
...
yt = ee, function() {
  ...
  function et(i, A, w, N, P) {
    ...
    for (C in w !== void 0 && (B = "" + w), function(v) {
      ...
      I.call(A, C) && !W.hasOwnProperty(C) && ($[C] = A[C]);
    if (i && i.defaultProps) {
    ...
    if (typeof i == "object") {
      if (Array.isArray(i))
        ...
        if (typeof P == "function" && P !== i.entries)
          for (var C, $ = P.call(i); !(C = $.next()).done; )
            Q(C.value) && pt(C.value, A);
      }
    }
  }
  function wt(i) {
