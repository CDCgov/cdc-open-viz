import { n as w, g as Z, T as me, F as pe, h as ve, i as ye } from "./storybook-index-9158acde.es.js";
import { R as f, r as b } from "./storybook-index-f2fed736.es.js";
import { g as ee } from "./storybook-_commonjsHelpers-c5d32002.es.js";
import { d as xe } from "./storybook-debounce-cc216a80.es.js";
import { i as we } from "./storybook-memoize-b2d53f3a.es.js";
...
var $ = function(e, t, n) {
  ...
  var t = e.onMove, n = e.onKey, r = V(e, ["onMove", "onKey"]), o = b.useRef(null), a = T(t), l = T(n), s = b.useRef(null), c = b.useRef(!1), u = b.useMemo(function() {
    var _ = function(v) {
      U(v), (O(v) ? v.touches.length > 0 : v.buttons > 0) && o.current ? a(A(o.current, v, s.current)) : E(!1);
    }, j = function() {
      return E(!1);
    };
    function E(v) {
      var m = c.current, x = L(o.current), N = v ? x.addEventListener : x.removeEventListener;
      N(m ? "touchmove" : "mousemove", _), N(m ? "touchend" : "mouseup", j);
    }
    return [function(v) {
      var m = v.nativeEvent, x = o.current;
  ...
  return { r: p(255 * [r, s, l, l, c, r][u]), g: p(255 * [c, r, r, s, l, l][u]), b: p(255 * [l, l, c, r, r, s][u]), a: p(o, 2) };
}, Ne = function(e) {
  ...
  var t = e.r, n = e.g, r = e.b, o = e.a, a = o < 1 ? z(p(255 * o)) : "";
  ...
  }, "aria-label": "Hue", "aria-valuenow": p(t), "aria-valuemax": "360", "aria-valuemin": "0" }, f.createElement(W, { className: "react-colorful__hue-pointer", left: t / 360, color: B({ h: t, s: 100, v: 100, a: 1 }) })));
  ...
  }, "aria-label": "Color", "aria-valuetext": "Saturation " + p(t.s) + "%, Brightness " + p(t.v) + "%" }, f.createElement(W, { className: "react-colorful__saturation-pointer", top: 1 - t.v / 100, left: t.s / 100, color: B(t) })));
}), le = function(e, t) {
  if (e === t)
    return !0;
  for (var n in e)
...
}, Fe = { defaultColor: "000", toHsva: _e, fromHsva: function(e) {
  return $e({ h: e.h, s: e.s, v: e.v, a: 1 });
}, equal: Re }, Ie = function(e) {
  return f.createElement(ze, k({}, e, { colorModel: Fe }));
}, He = function(e) {
  var t = e.className, n = e.hsva, r = e.onChange, o = { backgroundImage: "linear-gradient(90deg, " + H(Object.assign({}, n, { a: 0 })) + ", " + H(Object.assign({}, n, { a: 1 })) + ")" }, a = S(["react-colorful__alpha", t]), l = p(100 * n.a);
...
var at = w.div({ position: "relative", maxWidth: 250 }), lt = w(Z)({ position: "absolute", zIndex: 1, top: 4, left: 4 }), st = w.div({ width: 200, margin: 5, ".react-colorful__saturation": { borderRadius: "4px 4px 0 0" }, ".react-colorful__hue": { boxShadow: "inset 0 0 0 1px rgb(0 0 0 / 5%)" }, ".react-colorful__last-control": { borderRadius: "0 0 4px 4px" } }), ct = w(me)(({ theme: e }) => ({ fontFamily: e.typography.fonts.base })), it = w.div({ display: "grid", gridTemplateColumns: "repeat(9, 16px)", gap: 6, padding: 3, marginTop: 5, width: 200 }), ut = w.div(({ theme: e, active: t }) => ({ width: 16, height: 16, boxShadow: t ? `${e.appBorderColor} 0 0 0 1px inset, ${e.textMutedColor}50 0 0 0 4px` : `${e.appBorderColor} 0 0 0 1px inset`, borderRadius: e.appBorderRadius })), ft = `url('data:image/svg+xml;charset=utf-8,<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill-opacity=".05"><path d="M8 0h8v8H8zM0 8h8v8H0z"/></svg>')`, J = ({ value: e, active: t, onClick: n, style: r, ...o }) => {
...
}, ht = w(pe.Input)(({ theme: e }) => ({ width: "100%", paddingLeft: 30, paddingRight: 30, boxSizing: "border-box", fontFamily: e.typography.fonts.base })), dt = w(ve)(({ theme: e }) => ({ position: "absolute", zIndex: 1, top: 6, right: 7, width: 20, height: 20, padding: 4, boxSizing: "border-box", cursor: "pointer", color: e.input.color })), de = ((e) => (e.RGB = "rgb", e.HSL = "hsl", e.HEX = "hex", e))(de || {}), F = Object.values(de), gt = /\(([0-9]+),\s*([0-9]+)%?,\s*([0-9]+)%?,?\s*([0-9.]+)?\)/, bt = /^\s*rgba?\(([0-9]+),\s*([0-9]+),\s*([0-9]+),?\s*([0-9.]+)?\)\s*$/i, mt = /^\s*hsla?\(([0-9]+),\s*([0-9]+)%,\s*([0-9]+)%,?\s*([0-9.]+)?\)\s*$/i, K = /^\s*#?([0-9a-f]{3}|[0-9a-f]{6})\s*$/i, pt = /^\s*#?([0-9a-f]{3})\s*$/i, vt = { hex: Ie, rgb: Le, hsl: qe }, I = { hex: "transparent", rgb: "rgba(0, 0, 0, 0)", hsl: "hsla(0, 0%, 0%, 0)" }, Q = (e) => {
  ...
  let r = t.hex.match(pt);
  if (!r)
    return K.test(t.hex) ? t.hex : I.hex;
  let [o, a, l] = r[1].split("");
  return `#${o}${o}${a}${a}${l}${l}`;
}, xt = (e, t) => {
  ...
  let c = b.useMemo(() => yt(n, o, l).toLowerCase(), [n, o, l]), u = b.useCallback((h) => {
    ...
    let h = F.indexOf(l) + 1;
    h >= F.length && (h = 0), s(F[h]);
    let g = (o == null ? void 0 : o[F[h]]) || "";
    r(g), t(g);
  }, [o, l, t]);
  ...
  let s = b.useCallback(ot(n, 200), [n]), { value: c, realValue: u, updateValue: d, color: h, colorSpace: g, cycleColorSpace: _ } = xt(t, s), { presets: j, addPreset: E } = wt(a, h, g), v = vt[g];
  return f.createElement(at, null, f.createElement(lt, { startOpen: l, closeOnOutsideClick: !0, onVisibleChange: () => E(h), tooltip: f.createElement(st, null, f.createElement(v, { color: u === "transparent" ? "#000000" : u, onChange: d, onFocus: r, onBlur: o }), j.length > 0 && f.createElement(it, null, j.map((m, x) => f.createElement(Z, { key: `${m.value}-${x}`, hasChrome: !1, tooltip: f.createElement(ct, { note: m.keyword || m.value }) }, f.createElement(J, { value: m[g], active: h && q(m[g]) === q(h[g]), onClick: () => d(m.value) }))))) }, f.createElement(J, { value: u, style: { margin: 4 } })), f.createElement(ht, { id: ye(e), value: c, onChange: (m) => d(m.target.value), onFocus: (m) => m.target.select(), placeholder: "Choose color..." }), c ? f.createElement(dt, { icon: "markup", onClick: _ }) : null);
}, It = kt;
export {
  kt as ColorControl,
  It as default
