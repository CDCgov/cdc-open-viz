import { c as ye, g as ke } from "./storybook-_commonjsHelpers-BkfeUUK-.es.js";
var ge = { exports: {} };
/* @license
Papa Parse
v5.4.1
...
`, d = '"', A = d + d, k = !1, a = null, w = !1;
      ...
      if (typeof t == "string" && (t = JSON.parse(t)), Array.isArray(t)) {
        if (!t.length || Array.isArray(t[0])) return U(null, t, k);
        if (typeof t[0] == "object") return U(a || Object.keys(t[0]), t, k);
      ...
      function U(n, m, S) {
        ...
        if (T && i) {
          for (var I = 0; I < n.length; I++) 0 < I && (b += s), b += D(n[I], I);
          0 < m.length && (b += h);
        }
      ...
      function D(n, m) {
        ...
        return (S = S || r === !0 || typeof r == "function" && r(n, m) || Array.isArray(r) && r[m] || function(T, O) {
          for (var I = 0; I < O.length; I++) if (-1 < T.indexOf(O[I])) return !0;
          return !1;
...
`, '"', f.BYTE_ORDER_MARK], f.WORKERS_SUPPORTED = !Y && !!E.Worker, f.NODE_STREAM_INPUT = 1, f.LocalChunkSize = 10485760, f.RemoteChunkSize = 5242880, f.DefaultDelimiter = ",", f.Parser = oe, f.ParserHandle = de, f.NetworkStreamer = re, f.FileStreamer = ie, f.StringStreamer = V, f.ReadableStreamStreamer = ne, E.jQuery) {
      ...
      X.fn.parse = function(t) {
        ...
        return this.each(function(h) {
          if (!(X(this).prop("tagName").toUpperCase() === "INPUT" && X(this).attr("type").toLowerCase() === "file" && E.FileReader) || !this.files || this.files.length === 0) return !0;
          for (var d = 0; d < this.files.length; d++) r.push({ file: this.files[d], inputElem: this, instanceConfig: X.extend({}, e) });
        }), i(), this;
        function i() {
          if (r.length !== 0) {
            var h, d, A, k, a = r[0];
            if (p(t.before)) {
              ...
              if (typeof w == "object") {
                if (w.action === "abort") return h = "AbortError", d = a.file, A = a.inputElem, k = w.reason, void (p(t.error) && t.error({ name: h }, d, A, k));
    ...
    function B(t) {
      this._handle = null, this._finished = !1, this._completed = !1, this._halted = !1, this._input = null, this._baseIndex = 0, this._partialLine = "", this._rowCount = 0, this._start = 0, this._nextChunk = null, this.isFirstChunk = !0, this._completeResults = { data: [], errors: [], meta: {} }, (function(e) {
        ...
        if (!this._handle.paused() && !this._handle.aborted()) {
          ...
          if (ae) E.postMessage({ results: h, workerId: f.WORKER_ID, finished: A });
      ...
      function b() {
        return n && i && (I("Delimiter", "UndetectableDelimiter", "Unable to auto-detect delimiting character; defaulted to '" + f.DefaultDelimiter + "'"), i = !1), t.skipEmptyLines && (n.data = n.data.filter(function(o) {
          ...
          if (Array.isArray(n.data[0])) {
            for (var l = 0; T() && l < n.data.length; l++) n.data[l].forEach(o);
            n.data.splice(0, 1);
...
`), j = 1 < N.length && N[0].length < F[0].length;
          if (F.length === 1 || j) return `
`;
          for (var z = 0, y = 0; y < F.length; y++) F[y][0] === `
` && z++;
          return z >= F.length / 2 ? `\r
` : "\r";
        }(o, x)), i = !1, t.delimiter) p(t.delimiter) && (t.delimiter = t.delimiter(o), n.meta.delimiter = t.delimiter);
        else {
          var g = function(C, _, q, F, N) {
            ...
            for (var $ = 0; $ < N.length; $++) {
              var c = N[$], Z = 0, K = 0, G = 0;
              y = void 0;
              for (var H = new oe({ comments: F, delimiter: c, newline: _, preview: 10 }).parse(C), Q = 0; Q < H.data.length; Q++) if (q && S(H.data[Q])) G++;
              else {
    ...
    function oe(t) {
      ...
      if (t.escapeChar !== void 0 && (k = t.escapeChar), (typeof r != "string" || -1 < f.BAD_DELIMITERS.indexOf(r)) && (r = ","), s === r) throw new Error("Comment character same as delimiter");
...
`);
      ...
      this.parse = function(u, U, D) {
        ...
        if (t.header && !U) {
          ...
          for (var C in v) {
            ...
            var q = _, F = g[_] || 0;
            for (0 < F && (L = !0, q = _ + "_" + F), g[_] = F + 1; x.includes(q); ) q = q + "_" + F;
            x.push(q);
          }
          if (L) {
            var N = u.split(i);
        ...
        if (A || A !== !1 && u.indexOf(e) === -1) {
          for (var j = u.split(i), z = 0; z < j.length; z++) {
            if (o = j[z], a += o.length, z !== j.length - 1) a += i.length;
            else if (D) return M();
            if (!s || o.substring(0, b) !== s) {
              if (T) {
                if (O = [], G(o.split(r)), se(), w) return M();
              } else G(o.split(r));
              if (d && d <= z) return O = O.slice(0, d), M(!0);
            }
          }
          return M();
        }
        for (var y = u.indexOf(r, a), R = u.indexOf(i, a), $ = new RegExp(ee(k) + ee(e), "g"), c = u.indexOf(e, a); ; ) if (u[a] !== e) if (s && o.length === 0 && u.substring(a, a + b) === s) {
          if (R === -1) return M();
          a = R + S, R = u.indexOf(i, a), y = u.indexOf(r, a);
        } else if (y !== -1 && (y < R || R === -1)) o.push(u.substring(a, y)), a = y + m, y = u.indexOf(r, a);
        else {
          if (R === -1) break;
          if (o.push(u.substring(a, R)), J(R + S), T && (se(), w)) return M();
          if (d && O.length >= d) return M(!0);
        }
        else for (c = a, a++; ; ) {
          if ((c = u.indexOf(e, c + 1)) === -1) return D || I.push({ type: "Quotes", code: "MissingQuotes", message: "Quoted field unterminated", row: O.length, index: a }), Q();
          if (c === n - 1) return Q(u.substring(a, c).replace($, e));
          if (e !== k || u[c + 1] !== k) {
            if (e === k || c === 0 || u[c - 1] !== k) {
              ...
              if (u.substring(c + 1 + K, c + 1 + K + S) === i) {
                if (o.push(u.substring(a, c).replace($, e)), J(c + 1 + K + S), y = u.indexOf(r, a), c = u.indexOf(e, a), T && (se(), w)) return M();
                if (d && O.length >= d) return M(!0);
                break;
              }
    ...
    return ae && (E.onmessage = function(t) {
      var e = t.data;
      if (f.WORKER_ID === void 0 && e && (f.WORKER_ID = e.workerId), typeof e.input == "string") E.postMessage({ workerId: f.WORKER_ID, results: f.parse(e.input, e.config), finished: !0 });
