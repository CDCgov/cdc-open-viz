...
const TabVizConfig = ({ containerEl }) => {
  ...
  const visualizationList = {
    ...
    'data-bite': <CdcDataBite {...baseProps} />,
    'waffle-chart': <CdcWaffleChart {...baseProps} />,
    'markup-include': <CdcMarkupInclude {...baseProps} />
  }

  return type && visualizationList[type]
    ? (
      <Suspense fallback={<Ghost display="editor"/>}>
        {visualizationList[type]}
      </Suspense>
    )
