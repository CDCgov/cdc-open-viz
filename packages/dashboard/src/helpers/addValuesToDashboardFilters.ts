  ...
  const result = filters?.map((filter, index) => {
    ...
    if (filterValues.length > 0) {
      ...
      if (queryStringFilterValue) {
        ...
        // Priority: valid current active > defaultValue > reset label > first value
        if (isCurrentActiveValid) {
          // Keep the current active value if it's valid (user selection or saved value)
          filterCopy.active = currentActive
        } else if (filterCopy.defaultValue) {
          // Use defaultValue when current active is invalid or missing
