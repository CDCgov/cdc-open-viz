...
import customOrderDemoData from '../../../../.storybook/assets/custom-order-demo-data.csv?raw'
...
export const MultiVizConfigurationWorkflow: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    window.fetch = ((input, init) => {
      ...
      if (requestUrl.includes('custom-order-demo-data.csv')) {
        return Promise.resolve(
          new Response(customOrderDemoData, {
            status: 200,
    ...
    try {
      ...
      const configureButton = await canvas.findByRole(
        'button',
        { name: /configure your visualization/i },
        { timeout: 5000 }
      )
