      ...
      if (visualizationConfig.editing) {
        ...
        switch (visualizationConfig.type) {
          ...
          case 'data-byte':
            ...
            body = <><Header back={back} subEditor="Data Byte"/><CdcDataByte key={visualizationKey} config={visualizationConfig} isEditor={true} setConfig={updateConfig} isDashboard={true}/></>
            break
          case 'waffle-chart':
                ...
                {row.map((col, index) => {
                  if (col.width) {
                    ...
                    return <div className={`dashboard-col dashboard-col-${col.width}`} key={`vis__${index}`}>
                      ...
                      {visualizationConfig.type === 'data-byte' && <CdcDataByte key={col.widget} config={visualizationConfig} isEditor={false} setConfig={(newConfig) => {
                        updateChildConfig(col.widget, newConfig)
                      }} isDashboard={true}/>}
