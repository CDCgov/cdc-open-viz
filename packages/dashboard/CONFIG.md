...
| Field | Type | Required | Default | Description | Allowed values / Notes |
| --- | --- | --- | --- | --- | --- |
| `rows[].dashboardCondition.id` | `string` | No | Auto-generated | Internal dashboard-condition id used for filtered-data cache keys and stable condition identity. | The editor generates and preserves this id once dashboard-condition authoring is enabled. It is not an author-facing shared-filter target. |
...
| `rows[].columns[].conditionalWidgets[].dashboardCondition.id` | `string` | No | Auto-generated | Internal dashboard-condition id used for filtered-data cache keys and stable condition identity. | It is not an author-facing shared-filter target. Scope component conditions with the owning visualization key or row index instead. |
