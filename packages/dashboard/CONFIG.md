...
| `dashboard.sharedFilters[].filterBy` | `string` | No | `Query String` when missing or blank | URL filter behavior. | `Query String` or `File Name`. Only `File Name` enables filename rewrite behavior; missing or blank values are treated as Query String for legacy compatibility. |
