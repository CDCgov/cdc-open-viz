...
// Helpers
import calculateNodePosition from './helpers/calculateNodePosition.js'
...
const CdcWaffleChart = () => {
  ...
  const buildWaffle = useCallback(() => {
    ...
    // Calculate Node Animation Direction
    let prevVal = usePrevious(dataPercentage)
    let newVal = dataPercentage

    const calculateWaffleAnimation = (step) => {
    ...
    for (let i = 0; i < 100; i++) {
      let newNode = {
        shape: shape,
        x: calculateNodePosition(shape, 'x', i, nodeWidthNum, nodeSpacerNum),
        y: calculateNodePosition(shape, 'y', i, nodeWidthNum, nodeSpacerNum),
    ...
    return waffleData.map((node, key) => (
      node.shape === 'square'
        ? <Bar className="cove-waffle-chart__node"
               style={{ transitionDelay: calculateWaffleAnimation(key), fillOpacity: node.opacity }}
               x={node.x} y={node.y}
