...
describe('TrendArrow', () => {
  it('renders the up arrow asset with the default aria label', () => {
  ...
  it('renders the right arrow asset and preserves wrapper and label behavior', () => {
    ...
    const icon = screen.getByRole('img', { name: 'Trend no-change: Steady' })
    expect(icon).toHaveClass('cove-trend-arrow')
    expect(icon).not.toHaveClass('is-down')
    expect(icon).not.toHaveClass('is-no-change')
    expect(container.querySelector('svg')).toHaveAttribute('viewBox', '0 0 512 384')
    expect(container.querySelector('path')).toHaveAttribute(
      'd',
