/**
 * Cleans a data set by removing commas and $ signs from any numbers to try to plot the point,
 * and removing any data points that are NOT composed of all digits (but allow a decimal point).
 *
 * @param {Array} data - Array of data to be cleaned
 * @param {string} excludeKey - Indicates which key to NOT clean. Example: "Date" should not be cleaned if part of the data.
 * @param {boolean} [testing=false] - If true, logs the before and after data to the console.
 *
 * @returns {Array} - Returns the cleaned data as an array.
 *
 * @example
 * const cleanedData = cleanData(myData, 'Date');
 */
