...
export const publishAnalyticsEvent = <T extends ANALYTICS_EVENT_TYPES>({
  ...
  const resolvedVizTitle = vizTitle?.trim() ? vizTitle : 'No Title'
  const formattedEvent = `${app}|${vizTypeSubType}|${resolvedVizTitle}|${eventType}|${eventAction}|${specifics || 'no details'}`
