...
<Meta
  title="Hooks/Store/useVisConfig"
/>

# useVisConfig Hook
The useVisConfig hook acts as a configuration settings and data proxy for children COVE components.
...
<br/>
## Detailed Usage
The flow of the useVisConfig hook process performs the following steps:
1. The useVisConfig hook retrieves several configuration settings: `configObj`, `configUrl`, `defaults`, `runtime` <br/> *(see table above for specifics on each)*
