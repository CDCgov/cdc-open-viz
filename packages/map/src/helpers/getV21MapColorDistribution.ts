...
export const isV21MapColorDistributionEligible = (config: MapConfig, amount: number) => {
  return config.general?.palette?.version === '2.1' && amount <= 9 && Boolean(v2ColorDistribution[amount])
}

export const getV21MapColorDistribution = (config: MapConfig, amount: number): number[] | undefined => {
  if (!isV21MapColorDistributionEligible(config, amount)) return undefined
