...
export default function HexMapStatic({data}) {
  // const usa = topojson.feature(data, data.features[0].geometry);
  // const color = scaleQuantize({
  //   domain: [
  //     Math.min(...data.map((f) => f.geometry.coordinates.length)),
  //     Math.max(...data.map((f) => f.geometry.coordinates.length))
  //   ],
  //   range: [
  //     "#019ece",
  //     "#f4448b",
  ...
  const ref = useD3(
    (svg) => {
      ...
      const projection = d3.geoMercator()
        ...
        .translate([1100, 600]); // You have to play with these values to center your map
        debugger;
        ...
        .append("path")
        .attr("fill", "#69a2a2")
          // .attr("fill", color(feature.geometry.coordinates.length))
          .attr("d", d3.geoPath())
          .attr("stroke", "white")
        // Add the labels
        // svg.append("g")
