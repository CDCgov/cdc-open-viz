...
const EditorPanels = () => {
  ...
  const filtersJSX = config.filters.map((filter, index) => {
    ...
    return (
      <SectionBlock key={index}>
        ...
        <InputSelect label="Filter Column" options={columnsOptions} configField={[ 'filter', 'columnName' ]}
                     tooltip="Selecting a column will add a dropdown menu below the map legend and allow users to filter based on the values in this column."/>

        <InputSelect label="Filter Order" options={filterOptions} configField={[ 'filter', 'order' ]}
                     onChange={(e) => changeFilter(index, 'filterOrder', e.target.value)}/>
  ...
  const panelType = (
    <Accordion.Section label="Type">
      ...
      {(config.general.geoType === 'us' || config.general.geoType === 'us-county') && (
        <InputSelect label="Geography Subtype" options={[ 'US State-Level', 'US County-Level' ]} configField={[ 'general', 'geoType' ]}
                     onChange={e => handleEditorChanges('geoType', e.target.value)}/>
      )}


      {/* Select > Filter a config */}
      {config.general.geoType === 'single-config' && (
        <InputSelect label="State Selector" options={supportedStatesFipsCodes} configField={[ 'general', 'configPicked' ]}
                     onChange={e => handleEditorChanges('chooseState', e.target.value)}/>
      )}

      {/* Map Type */}
      <InputSelect label="Map Style" configField={[ 'general', 'type' ]}
      ...
      {'us' === config.general.geoType && 'data' === config.general.type && (
        <InputCheckbox label="Display As Hex Map" configField={[ 'general', 'displayAsHex' ]}/>
      )}

      {'us' === config.general.geoType && 'bubble' !== config.general.type && false === config.general.displayAsHex && (
        <InputCheckbox label="Display config labels" configField={[ 'general', 'displayStateLabels' ]}/>
      )}
    </Accordion.Section>
  )
  ...
  const panelGeneral = (
    <Accordion.Section label="General">
      <InputText label="Title" configField={[ 'general', 'title' ]} placeholder="Map Title"
                 tooltip="For accessibility reasons, you should enter a title even if you are not planning on displaying it."/>
      ...
      <InputText type="textarea" label="Subtext" configField={[ 'general', 'subtext' ]}
      ...
      {'us' === config.general.geoType &&
        <InputText label="Territories Label" configField={[ 'general', 'territoriesLabel' ]} placeholder="Territories"/>
      }
      {/*
  ...
  const panelColumns = (
    <Accordion.Section label="Columns">
      <SectionWrapper>
        <InputSelect label="Geography" options={columnsOptions} configField={[ 'columns', 'geo', 'name' ]}
                     tooltip="Select the source column containing the map location names or, for county-level maps, the FIPS codes."/>
        <InputCheckbox label="Hide Geography Column Name in Tooltip" configField={[ 'general', 'hideGeoColumnInTooltip' ]}/>
        <InputText label="Geography Label" configField={[ 'general', 'geoLabelOverride' ]} placeholder="Geography Label"
                   tooltip="Enter a geography label for use in tooltips."/>
      </SectionWrapper>

      {'navigation' !== config.general.type && (
        <SectionWrapper>
          <InputSelect label="Data Column" options={columnsOptions} configField={[ 'columns', 'primary', 'name' ]}
                       tooltip="Select the source column containing the categorical or numeric values to be mapped."/>
          <InputCheckbox label="Hide Data Column Name in Tooltip" configField={[ 'general', 'hidePrimaryColumnInTooltip' ]}/>
          <InputText label="Data Label" configField={[ 'columns', 'primary', 'label' ]} placeholder="Data Label"
                     tooltip="Enter a data label for use in tooltips and the data table."/>
      ...
      {config.general.type === 'bubble' && config.legend.type === 'category' && (
        <SectionWrapper>
          <InputSelect label="Category Column" options={columnsOptions} configField={[ 'columns', 'categorical', 'name' ]}
                       tooltip="Select the source column containing the categorical bubble values to be mapped."/>
        </SectionWrapper>
      )}
      ...
      {'navigation' !== config.general.type && (
        <SectionWrapper>
          <Label tooltip="For secondary values such as 'NA', the system can automatically color-code them in shades of gray, one shade for each special class.">
            Special Classes
          </Label>
          ...
          {specialClasses.map((specialClass, i) => (
            <SectionBlock>
              ...
              <div><p className="cove-heading--5 mb-1">Special Class {i + 1}</p></div>
              <InputSelect label="Data Key" options={columnsOptions} value={specialClass.key}
                           onChange={e => {
                             editColumn('primary', 'specialClassEdit', { prop: 'key', index: i, value: e.target.value })
                           }}
              />
              <InputSelect label="Value" options={columnsByKey[specialClass.key] && columnsByKey[specialClass.key].sort()}
          ...
          <Button onClick={(e) => editColumn('primary', 'specialClassAdd', {})} fluid>Add Filter</Button>
      ...
      {'navigation' !== config.general.type && (
        <>
          <Label tooltip="You can specify additional columns to display in tooltips and / or the supporting data table.">
            Additional Columns
          </Label>
          ...
          <Button onClick={() => addAdditionalColumn(additionalColumns.length + 1)} fluid>Add Column</Button>
        </>
      )}
  ...
  const panelDataTable = (
    <Accordion.Section>
      <InputText label="Data Table Title" placeholder="Data Table" configField={[ 'dataTable', 'title' ]}
                 tooltip="Label is required for Data Table for 508 Compliance"/>


      <InputText label="Index Column Header" placeholder="Location" configField={[ 'dataTable', 'indexLabel' ]}
                 tooltip="To comply with 508 standards, if the first column in the data table has no header, enter a brief one here."/>

      <InputText label="Data Table Caption" placeholder="Data Table" configField={[ 'dataTable', 'caption' ]}
                 tooltip="Enter a description of the data table to be read by screen readers."/>

      <InputCheckbox label="Show Table" configField={[ 'dataTable', 'forceDisplay' ]}
                     tooltip="Data tables are required for 508 compliance. When choosing to hide this data table, replace with your own version."/>

      <InputCheckbox label="Limit Table Height" configField={[ 'dataTable', 'limitHeight' ]}/>


      {config.dataTable.limitHeight &&
        <InputText label="Data Table Height" placeholder="Height(px)" configField={[ 'dataTable', 'height' ]} type="number" min="0" max="500"/>
      }
  ...
  const panelVisual = (
    <Accordion.Section label="Visual">
      ...
      {'navigation' === config.general.type && (
        <InputCheckbox label="Add border around map" configField={[ 'general', 'fullBorder' ]}/>
      )}

      <InputSelect label="Geo Border Color" configField={[ 'general', 'geoBorderColor' ]} options={{
      ...
      {'us-geocode' === config.general.type && (
        <InputText type="number" label="Geocode Circle Size" configField={[ 'visual', 'geoCodeCircleSize' ]} min="0" max="10"/>
      )}
      ...
      {(config.general.geoType === 'world' || (config.general.geoType === 'us' && config.general.type === 'bubble')) && (
        <InputCheckbox label="Show Data with Zero's on Bubble Map" configField={[ 'visual', 'showBubbleZeros' ]}/>
      )}

      {config.general.geoType === 'world' && (
        <InputCheckbox label="Allow Map Zooming" configField={[ 'general', 'allowMapZoom' ]}/>
      )}

      {config.general.type === 'bubble' && (
        <InputCheckbox label="Bubble Map has extra border" configField={[ 'visual', 'extraBubbleBorder' ]}/>
      )}

      {config.general.geoType === 'us' ||
        (config.general.geoType === 'us-county' && (
          <InputSelect label="City Style" configField={[ 'visual', 'cityStyle' ]} options={{
            circle: 'Circle',
            pin: 'Pin'
          }}/>
        ))}
