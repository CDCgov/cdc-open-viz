...
function CountyMapChecks(prevState, nextState) {
  ...
  const equalCountyCensusYear = prevState.state.general.countyCensusYear === nextState.state.general.countyCensusYear
  return equalData && equalBorderColors && equalLegend && equalColumnName && equalNavColumn && equalNumberOptIn && equalTooltipBehavior && equalCountyCensusYear ? true : false
}

const CountyMap = props => {
  const { state, runtimeLegend, runtimeFilters, applyTooltipsToGeo, data, geoClickHandler, applyLegendToRow, displayGeoName, containerEl, handleMapAriaLabels } = props
