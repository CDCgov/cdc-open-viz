...
| `general.territoriesAlwaysShow` | `boolean` | No | `true` | Controls how U.S. territories are rendered on U.S. maps. | `true`, `false`. On county maps saved before `4.26.4-1`, `false` still renders territory counties when matching filtered data exists; on county maps saved at `4.26.4-1` or later, `false` hides territory counties and `true` shows all supported territory geography. On state maps, `false` renders only territories with matching runtime data; `true` renders all supported territories whether data exists or not. When omitted, the editor/runtime treat it as `true`, and legacy configs are backfilled during migration. |
