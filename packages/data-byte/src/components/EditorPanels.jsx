...
const EditorPanels = () => {
  ...
  const updateDynamicImage = (name, index, subindex = null, value) => {
    ...
    configActions.updateConfig({ ...config, imageData: payload })
  }
  ...
  const panelGeneral = (
    <Accordion.Section label="General">
      <InputSelect label="Data Byte Style" options={BYTE_LOCATIONS} configField="byteStyle" initialDisabled/>

      {[ 'graphic' ].includes(config.byteStyle) &&
        <InputSelect label="Graphic Position" options={IMAGE_POSITIONS} configField="bytePosition" initialDisabled/>
      }

      <InputText label="Title" configField="title" placeholder="Data Byte Title"/>

      <InputText label="Message" configField="byteBody" type="textarea" tooltip={<>
        <p>
      ...
      <InputText label="Subtext/Citation" configField="subtext" placeholder="Data Byte Subtext or Citation" tooltip={<>
        <p>
  ...
  const panelVisual = (
    <Accordion.Section label="Visual">
      <InputText label="Byte Font Size" configField="byteFontSize" type="number" min="16" max="65"/>
  ...
  const panelImages = (
    <Accordion.Section label={`Image${[ 'dynamic' ].includes(config.imageData.display) ? 's' : ''}`}>
      ...
      <InputSelect label="Image/Graphic Position" options={IMAGE_POSITIONS} configField="bytePosition" initialDisabled/>
  ...
  return (
    <>
      ...
      {panelData}
      {panelFilters}
      {[ 'title', 'body' ].includes(config.byteStyle) && panelImages}
      {panelVisual}
    </>
  )
}
