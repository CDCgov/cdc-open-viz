...
const DataBite = () => {
  ...
  const DataImage = useCallback(() => {
    ...
    return (
      imageSource.length > 0 && 'graphic' !== biteStyle && 'none' !== imageData.display ?
  ...
  const Callout = () => (
    <div className="cove-data-byte__callout">
      <CircleCallout theme={config.theme} text={calculateDataBite()} byteFontSize={biteFontSize} dataFormat={dataFormat}/>
    </div>
  )
    ...
    <Component className="cove-data-byte" title={title} theme={theme}>
      {!missingRequiredSections && !config.newViz && (<>
        <div className={`cove-data-byte__container${fontSize ? ' font-' + fontSize : ''}`} flow={byteFlow}>
          {showByte && 'graphic' === biteStyle && isTop && <Callout/>}
          {isTop && <DataImage/>}
          <div className="cove-data-byte__content">
            {showByte && 'title' === biteStyle &&
              <div className="cove-data-byte__value" style={{ fontSize: biteFontSize + 'px' }}>{calculateDataBite()}</div>
            }
            <p className="cove-data-byte__text">
              {showByte && 'body' === biteStyle &&
                <span className="cove-data-byte__value mr-1" style={{ fontSize: biteFontSize + 'px' }}>{calculateDataBite()}</span>
              }
              {parse(biteBody)}
            </p>
            {subtext &&
