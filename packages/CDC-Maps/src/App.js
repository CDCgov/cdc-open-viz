...
class cdcMap extends Component {
    ...
    applyColorToLegend (
        ...
        let legendItemIndex = this.state.legend.data.indexOf(legend) - numberOfSpecialClasses
    ...
    buildLegend (newData = this.state.data, legendObject = this.state.legend, setStateInMethod = true) {
        ...
        const getIntervalAmt = () => {
            ...
            allGeoList.forEach((obj) => {
                ...
                if( false === Number.isNaN( parseFloat(newData[obj][datacol]) ) ) {
                    allNums.set(newData[obj][statecol], parseFloat(newData[obj][datacol]))
                }
            ...
            let interval = (Math.max(...allNums.values()) - Math.min(...allNums.values())) / number

            interval = Math.round(interval * 10) / 10

            // Returns min value, max value and the calculated interval between them
            return [Math.min(...allNums.values()), Math.max(...allNums.values()), interval, allNums]
        }
        ...
        switch (type) {
            ...
            case 'category':
                ...
                usedAlready.forEach((val) => {
                    ...
                    if( this.isSpecialClassValue(val, legendObject.specialClasses) && false === specialClassesUsed.includes(val) ) {
    ...
    processData (unifiedLegend = false) {
        ...
        if (this.state.data[0][geoColumnName] &&
        ...
        for (let i = 0; i < rawData.length; i++) {
            ...
            if (checkAgainst.includes(item[geoColumnName])) {
                ...
                if (false === this.applyLegendToValue(item[this.state.columns.primary.name], item[geoColumnName])) {
                    continue
                }
    ...
    processDataTableOnRender () {
        ...
        Object.keys(this.state.columns).forEach( (column, index) => {
            ...
            if (true === this.state.columns[column].dataTable && "" !== this.state.columns[column].name) {
                const newCol = {
                    Header: this.state.columns[column].label ||
                ...
                if (0 === index) { // Only do this for the first column, which is the geo column.
                    newCol.Cell = (row) => {
                        // Grab color
                        let legendColor = this.applyLegendToValue(
                            row.original[this.state.columns.primary.name], row.original[this.state.columns.geo.name])
                        ...
                        let cellMarkup = (
                            ...
                            <span className="legend-color"
                                  style={{ backgroundColor: legendColor[0] }}></span>
                                            {labelValue}
                            </span>
                        )
    ...
    getCellLabelValue(row) {
        ...
        if (this.geoAbbreviations.includes(row.value)) {
            labelValue = this.geoNames[this.geoAbbreviations.indexOf(
                row.value)]
        }
    ...
    getCellAnchor(markup, row) {
        ...
        if (this.state.columns.navigate && row.original[this.state.columns.navigate.name]) {
            markup = (<span>
                <a href={this.parseLinks(row.original[this.state.columns.navigate.name])}
                    title={'Click for more information'}
                    rel="noopener noreferrer"
                    target="_parent"
                    onClick={ (e) => this.geoClickHandler(e, row.original[this.state.columns.navigate.name]) }
                    aria-label={'Click for more information'}> </a>
                    {markup}
                    <span
                        className="more-information">Learn More
                    </span>
            </span>)
        }
    ...
    processNonMapGeos (value) {
        ...
        if (this.supportedTerritories.includes(value) && false ===
            this.territoriesInData.includes(value)) {
            this.territoriesInData.push(value)
        }

        if (this.supportedCities.includes(value) && false ===
            this.citiesInData.includes(value)) {
    ...
    async componentDidMount () {
        ...
        if(initialState.general && initialState.general.backgroundColor) {
            document.body.style.backgroundColor = initialState.general.backgroundColor;
        }
        ...
        if (initialState.dataUrl) {
            ...
            if(true === this.isEditor || true === this.state.editor.active) { // The second part of this is only for debugging
        ...
        Object.keys(initialState).forEach( (key) => {
            if("object" === typeof initialState[key] && false === Array.isArray(initialState[key])) {
                if(this.state[key] ) {
        ...
        if (initialState.filters) {
            ...
            filterList.forEach((filter, index) => {
                ...
                const filterValues = this.generateValuesForFilter(filter, initialState.data)
    ...
    async handleEditorChanges(property, value) {
        ...
        switch (property) {
            ...
            case 'legendType':
                ...
                await this.setState( (prevState) => {
                    return {
                        legend: this.buildLegend(this.processData( this.state.legend.unified ), {...this.state.legend, type: value}, false)
                    }
                })
            ...
            case 'legendNumber':
                ...
                await this.setState( (prevState) => {
                    return {
                        legend: this.buildLegend(this.processData( this.state.legend.unified ), {...this.state.legend, numberOfItems: parseInt(value)}, false)
                    }
                })
            ...
            case 'unifiedLegend':
                await this.setState( (prevState) => {
                    return {
                        legend: this.buildLegend(this.processData( value ), {...this.state.legend, unified: value}, false)
                    }
                })
            break;
            ...
            case 'categoryOrder':
                await this.setState( (prevState) => {
                    return {
                        legend: {
                            ...prevState.legend,
                            categoryValuesOrder: arrayMove(prevState.legend.categoryValuesOrder, value[0], value[1])
                        }
                    }
                })
    ...
    async editorLoadData() {
        ...
        switch(type) {
            case 'local':
                this.setState({columns: resetObj, data: this.state.editor.localFileData})
            break;
            case 'url':
    ...
    async editColumn(columnName, editTarget, value) {
        switch (editTarget) {
            case 'specialClassDelete':
                const updatedSpecialClasses = Array.from(this.state.legend.specialClasses)

                updatedSpecialClasses.splice(value, 1)

                await this.setState( (prevState) => {
                    return {
                        legend: this.buildLegend(this.processData( value ), {...this.state.legend, specialClasses: updatedSpecialClasses}, false)
                    }
                })
    ...
    render () {
        ...
        if ('bottom' === this.state.legend.position || this.state.legend.showLegend === false) {
            ...
            <React.Fragment>
                ...
                <section className={true === this.state.editor.expanded ? "editor-active full-container" : "full-container"}>
                    <ReactTooltip
                        ...
                        html={true}
                        className={'tooltip'}/>
                    {this.state.editor.expanded === false && true === this.state.editor.active && false === this.state.general.showTitle && <button className="editor-toggle no-header collapsed" title="Expand Editor" onClick={() => { this.toggleEditor() }}></button>}
                    ...
                    <header>
                        <h1 className={'site-title ' + this.state.general.headerColor}
                            ...
                            {this.state.editor.expanded === false && true === this.state.editor.active && <button className="editor-toggle collapsed" title="Expand Editor" onClick={() => { this.toggleEditor() }}></button>}
                            { ReactHtmlParser(this.state.general.title) }
                        </h1>
                    </header>
                    }
                    <main className={mapContainerClasses}>
                        <section className={"geography-container " + this.state.general.geoType} aria-hidden="true">
                                                    ...
                                                    geos.map((geo, i) => {
                                                        ...
                                                        if (geoData !== undefined) {
                                                            ...
                                                            legendColors = this.applyLegendToValue(
                                                                processedData[geoName][this.state.columns.primary.name], processedData[geoName][this.state.columns.geo.name])
                                                        ...
                                                        if (legendColors) {
                                                            ...
                                                            const toolTip = this.applyTooltipsToGeo(geoName,
                                                                geoData)
                                                            let renderedGeo = (
                                                                <Geography
                                                                    data-tip={toolTip}
                                                                    ...
                                                                    id={geoName}
                                                                    tabIndex={-1}
                                                                    className={'rsm-geography ' + this.state.general.geoBorderColor}
                                                                    key={geoName}
                                                            ...
                                                            if (this.state.columns.navigate &&
                                                                ...
                                                                renderedGeo = (
                                                                    <a href={this.parseLinks(geoData[this.state.columns.navigate.name])}
                                                                       key={i}
                                                                       onClick={ (e) => this.geoClickHandler(e, geoData[this.state.columns.navigate.name]) }
                                                                       target="_parent"
                                                                       tabIndex={-1}
                                                                       rel="noopener noreferrer">{renderedGeo}</a>)
                                                            }
                            ...
                            <section className="cities-territories">
                                ...
                                <SquareGeo type="Territories" state={this.state}
                                           ...
                                           applyTooltipsToGeo={(
                                               ...
                                               geoName, data)}
                                           applyLegendToValue={(value, geoName) => this.applyLegendToValue(
                                               value, geoName)}/>
                                }
                        ...
                        <aside>
                            <Sidebar legend={this.state.legend} filters={this.state.filters}
                    ...
                    <section className={'data-table'}>
                        <h3 className={this.state.general.expandDataTable ? '' : 'collapsed'}
                            onClick={() => { this.toggleDataTableVisibility() }}>Data Table</h3>
                        <ReactTable
                        ...
                        <CSVLink data={this.state.data}
                                 ...
                                 className={this.state.general.headerColor +
                                 ' btn btn-primary btn-download no-border'}>Download Data (CSV)</CSVLink>}
                    </section>
                    }
