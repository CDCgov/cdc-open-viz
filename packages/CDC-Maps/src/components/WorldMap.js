...
const { state, applyTooltipsToGeo, processedData, geoClickHandler, applyLegendToValue, displayGeoName, countryValues } = props
...
const geoList = (geographies) => {
    ...
    if(true === state.general.hasRegions && state.columns.primary.name.length > 0 && state.columns.geosInRegion.name.length > 0) {
        ...
        regions.forEach( (regionName, i) => {
            ...
            if (legendColors) {
                ...
                geosInRegion.forEach((name) => {
                    ...
                    let tempPath = interpolatePath(regionPath, geoPaths)
...
return (
    <React.Fragment>
        {'data' === state.general.type && ZoomControls}
