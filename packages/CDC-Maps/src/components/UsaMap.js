...
export default (props) => {
    ...
    const { state, applyTooltipsToGeo, processedData, geoClickHandler, applyLegendToValue, displayGeoName, supportedStates } = props

    const [ territoriesData, setTerritoriesData ] = useState([])

    useEffect(() => {
        let territoriesKeys = Object.keys(supportedTerritories) // processedData will have already mapped abbreviated territories to their full names

        let dataKeys = Object.keys(processedData)

        // Territories need to show up if they're in the data at all, not just if they're "active". That's why this is different from Cities
        let territoriesList = dataKeys.filter((name) => territoriesKeys.includes(name))
    ...
    territoriesData.forEach( (territory) => {
        ...
        if(territoryData) {
            ...
            if(legendColors) {
                ...
                if( (state.columns.navigate && territoryData[state.columns.navigate.name]) || 'click' === state.tooltips.appearanceType) {
                    needsPointer = true
                }
    ...
    const geoList = (geographies) => {
        ...
        if(true === state.general.hasRegions && state.columns.geosInRegion.name.length > 0) {
            ...
            regions.forEach( (regionName) => {
                ...
                if (legendColors) {
                    ...
                    if ( (state.columns.navigate && regionData[state.columns.navigate.name] ) || 'click' === state.tooltips.appearanceType) {
        ...
        const geosJsx = geographies.map((geo) => {
            ...
            let geoData = processedData[geoKey]
