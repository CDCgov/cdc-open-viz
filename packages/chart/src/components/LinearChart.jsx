  ...
  return isNaN(width) ? (
    ...
    <ErrorBoundary component='LinearChart'>
      <svg width={width} height={config.showChartBrush ? height * 1.3 : height} className={`linear ${config.animate ? 'animated' : ''} ${animatedChart && config.animate ? 'animate' : ''}`} role='img' aria-label={handleChartAriaLabels(config)} tabIndex={0} ref={svgRef}>
        ...
        {/* brush */}
        {/* config.showChartBrush && (config.visualizationType === 'Area Chart' || config.visualizationType === 'Combo') && <AreaChart xScale={xScale} yScale={yScale} yMax={yMax} xMax={xMax} chartRef={svgRef} isDebug={isDebug} isBrush={true} /> */}
