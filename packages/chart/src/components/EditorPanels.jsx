...
const Regions = memo(() => {
  ...
  return (
    <>
      ...
      <Button className='mb-2' onClick={regionAddColumn} fluid>
        Add Region
      </Button>
    </>
  )
...
const EditorPanels = () => {
  ...
  const panelGeneral = (
    <Accordion.Section label='General'>
      ...
      {(config.visualizationType === 'Bar' || config.visualizationType === 'Combo') && (
        <InputSelect
          label='Chart Subtype'
          configField={'visualizationSubType'}
          options={{
            regular: 'Regular',
            stacked: 'Stacked'
          }}
      ...
      {config.visualizationType === 'Bar' && <InputSelect label='Bar Style' configField={'barStyle'} options={showBarStyleOptions()} />}

      {config.visualizationType === 'Bar' && config.barStyle === 'rounded' && <InputSelect label='Tip Rounding' configField={'tipRounding'} options={['top', 'full']} />}

      {config.visualizationType === 'Bar' && config.barStyle === 'rounded' && <InputSelect label='Rounding Style' configField={'roundingStyle'} options={['standard', 'shallow', 'finger']} />}

      {config.visualizationType === 'Bar' && config.orientation === 'horizontal' && <InputSelect label='Label Placement' configField={['yAxis', 'labelPlacement']} options={['Below Bar', 'On Date/Category Axis']} />}
      ...
      {config.orientation === 'horizontal' && (config.yAxis.labelPlacement === 'Below Bar' || config.yAxis.labelPlacement === 'On Date/Category Axis' || config.visualizationType === 'Paired Bar') ? (
        ...
        config.visualizationType !== 'Pie' && <InputCheckbox label='Display label on data' configField={'labels'} />
      )}

      {config.visualizationType === 'Pie' && <InputSelect label='Pie Chart Type' configField={'pieType'} options={['Regular', 'Donut']} />}
      ...
      <InputText type='textarea' label='Subtext' configField={'description'} placeholder='Subtext' tooltip='Enter supporting text to display below the data visualization, if applicable. The following HTML tags are supported: strong, em, sup, and sub.' />

      <InputText type='textarea' label='Footnotes' configField={'footnotes'} placeholder='Footnotes' />

      {config.orientation === 'vertical' && <InputText type='number' label='Chart Height' configField={['heights', 'vertical']} />}
    </Accordion.Section>
  )
  ...
  const panelDataSeries = (
    <Accordion.Section label='Data Series' warnIf={!config.series || config.series.length === 0 || (config.visualizationType === 'Paired Bar' && config.series.length < 2)}>
      {(!config.series || config.series.length === 0) && config.visualizationType !== 'Paired Bar' && <div className='cove-alert cove-alert--error'>At least one series is required</div>}

      {(!config.series || config.series.length === 0 || config.series.length < 2) && config.visualizationType === 'Paired Bar' && <div className='cove-alert cove-alert--error'>Select two data series for paired bar chart (e.g., Male and Female).</div>}
      ...
      {config.series && config.series.length === 1 && <InputSelect label='Rank by Value' configField='sortSeries' options={['asc', 'desc']} onChange={e => sortSeries(e.target.value)} />}
    </Accordion.Section>
  )
  ...
  const panelLeftValueAxisOrFormat = (
    <Accordion.Section label={config.visualizationType !== 'Pie' ? (config.orientation !== 'horizontal' ? 'Left Value Axis' : 'Value Axis') : 'Data Format'} warnIf={config.visualizationType === 'Pie' && !config.yAxis.dataKey}>
      {config.visualizationType === 'Pie' && <InputSelect label='Data Column' configField={['yAxis', 'dataKey']} options={getDataColumns(config.data)} tooltip='Select the source data to be visually represented.' required={true} />}

      {config.visualizationType !== 'Pie' && (
        <>
          <InputText label='Label' configField={['yAxis', 'label']} />

          {config.runtime.seriesKeys && config.runtime.seriesKeys.length === 1 && <InputCheckbox label='Use Legend Value in Hover' configField={'isLegendValue'} />}
          ...
          {config.visualizationType === 'Paired Bar' && <InputText type='number' label='Tick rotation (Degrees)' configField={['yAxis', 'tickRotation']} min={0} />}

          <InputText
            type='number'
            label={config.orientation === 'horizontal' ? 'Size (Height)' : 'Size (Width)'}
            configField={['yAxis', 'size']}
          ...
          {config.orientation === 'horizontal' && <InputText type='number' label='Label offset' configField={['xAxis', 'labelOffset']} />}

          {config.orientation !== 'horizontal' && <InputCheckbox label='Display Gridlines' configField={['yAxis', 'gridLines']} />}
        </>
      )}
      ...
      <SectionWrapper label='Number Formatting'>
        ...
        <div className='cove-grid cove-grid--gap--2'>
          <div className='cove-grid__col--6'>
            <InputText label='Prefix' configField={['dataFormat', 'prefix']} tooltip={config.visualizationType === 'Pie' ? <p>Enter a data prefix to display in the data table and chart tooltips, if applicable.</p> : <p>Enter a data prefix (such as "$"), if applicable.</p>} />
          </div>
          <div className='cove-grid__col--6'>
            <InputText label='Suffix' configField={['dataFormat', 'suffix']} tooltip={config.visualizationType === 'Pie' ? <p>Enter a data suffix to display in the data table and chart tooltips, if applicable.</p> : <p>Enter a data suffix (such as "%"), if applicable.</p>} />
          </div>
        </div>
  ...
  const panelRightValueAxis = (
    <Accordion.Section label='Right Value Axis'>
      ...
      <SectionWrapper label='Number Formatting'>
        ...
        <div className='cove-grid cove-grid--gap--2'>
          <div className='cove-grid__col--6'>
            <InputText label='Prefix' configField={['dataFormat', 'rightPrefix']} tooltip={config.visualizationType === 'Pie' ? <p>Enter a data prefix to display in the data table and chart tooltips, if applicable.</p> : <p>Enter a data prefix (such as "$"), if applicable.</p>} />
          </div>
          <div className='cove-grid__col--6'>
            <InputText label='Suffix' configField={['dataFormat', 'rightSuffix']} tooltip={config.visualizationType === 'Pie' ? <p>Enter a data suffix to display in the data table and chart tooltips, if applicable.</p> : <p>Enter a data suffix (such as "%"), if applicable.</p>} />
          </div>
        </div>
  ...
  const panelDateCategoryAxisSegments = (
    <Accordion.Section label={config.visualizationType !== 'Pie' ? (config.visualizationType === 'Bar' ? 'Date/Category Axis' : 'Date/Category Axis') : 'Segments'} warnIf={!config.xAxis.dataKey}>
      {config.visualizationType !== 'Pie' && (
        <>
          <InputSelect label='Data Type' configField={['xAxis', 'type']} options={config.visualizationType !== 'Scatter Plot' ? ['categorical', 'date'] : ['categorical', 'date', 'continuous']} />
          <InputSelect label='Data Key' configField={['xAxis', 'dataKey']} options={getDataColumns(config.data)} tooltip='Select the column or row containing the categories or dates for this axis.' required={true} />
        </>
      )}

      {config.visualizationType === 'Pie' && (
        <InputSelect label='Segmented Labels' configField={['xAxis', 'dataKey']} options={getDataColumns(config.data)} tooltip="Select the source row or column that contains the segment labels. Depending on the data structure, it may be listed as 'Key'." required={true} />
      )}
      ...
      {config.visualizationType !== 'Pie' && (
        <>
          ...
          <InputCheckbox label={config.xAxis.type === 'date' ? 'Limit by start and/or end dates' : 'Exclude one or more values'} configField={['exclusions', 'active']} tooltip='When this option is checked, you can select source-file values for exclusion from the date/category axis.' />
          ...
          {config.yAxis.labelPlacement !== 'Below Bar' && <InputText type='number' label='Tick rotation (Degrees)' configField={['xAxis', 'tickRotation']} min='0' />}
      ...
      {config.visualizationType === 'Pie' && (
        <>
          <InputCheckbox label='Exclude one or more values' configField={['exclusions', 'active']} tooltip='When this option is checked, you can select source-file values for exclusion from the pie segments.' />
  ...
  const panelLegend = (
    <Accordion.Section label='Legend'>
      ...
      */}
      ...
      {config.visualizationType === 'Bar' && config.visualizationSubType === 'regular' && config.runtime.seriesKeys.length === 1 && <InputSelect label='Color code by category' configField={['legend', 'colorCode']} options={getDataValueOptions(config.data)} />}
      ...
      {config.legend.position === 'bottom' && <InputCheckbox label='Single Row Legend' configField={['legend', 'singleRow']} />}
  ...
  const panelFilters = (
    <Accordion.Section label='Filters'>
      {config.filters &&
        config.filters.map((filter, index) => (
          <SectionBlock key={index}>
            ...
            <InputSelect label='Filter' value={filter.columnName} options={getFilteredDataColumns(config, config.data)} onChange={e => updateFilterProp('columnName', index, e.target.value)} />
      ...
      <Button className='mb-2' onClick={addNewFilter} fluid>
        Add Filter
      </Button>
    </Accordion.Section>
  )
  ...
  const panelVisual = (
    <Accordion.Section label='Visual'>
      ...
      {config.visualizationType !== 'Box Plot' && config.series?.some(series => series.type === 'Bar' || series.type === 'Paired Bar') && <InputSelect label='Bar Borders' configField='barHasBorder' options={['true', 'false']} />}
      ...
      {config.visualizationType !== 'Pie' && <InputText type='number' label='Data Cut-off Label' configField='dataCutoffLabel' tooltip="The label for the special 'less than' category. This option supports special conditions like suppressed data." />}

      {config.orientation === 'horizontal' && !config.isLollipopChart && config.yAxis.labelPlacement !== 'On Bar' && <InputText type='number' label='Bar Thickness' configField='barHeight' min='15' />}

      {((config.visualizationType === 'Bar' && config.orientation !== 'horizontal') || config.visualizationType === 'Combo') && <InputText type='number' label='Bar Thickness' configField='barHeight' min='15' />}

      {(config.orientation === 'horizontal' || config.visualizationType === 'Paired Bar') && <InputText type='number' label='Bar Space' configField='barSpace' min='0' />}

      {(config.visualizationType === 'Bar' || config.visualizationType === 'Line' || config.visualizationType === 'Combo') && <InputCheckbox label='Add Top Axis Line' configField={['topAxis', 'hasLine']} />}

      {(config.visualizationType === 'Line' || config.visualizationType === 'Combo') && <InputCheckbox label='Append Series Name to End of Line Charts' configField='showLineSeriesLabels' />}

      {(config.visualizationType === 'Line' || config.visualizationType === 'Combo') && config.showLineSeriesLabels && <InputCheckbox label='Match Series Color to Name at End of Line Charts' configField='colorMatchLineSeriesLabels' />}
    </Accordion.Section>
  )

  const panelDataTable = (
    <Accordion.Section label='Data Table'>
      <InputCheckbox label='Show Table' configField={['table', 'show']} tooltip='Hiding the data table may affect accessibility. An alternate form of accessing visualization data is a 508 requirement.' />

      <InputText type='textarea' label='Data Table Caption' configField={['table', 'caption']} tooltip='Enter a description of the data table to be read by screen readers.' />

      <InputToggle label='Limit Table Height' configField={['table', 'limitHeight']} />

      {config.table.limitHeight && <InputText type='number' label='Data Table Height' configField={['table', 'height']} min='0' max='500' placeholder='Height (px)' />}
      ...
      {config.visualizationType !== 'Pie' && <InputText label='Index Column Header' configField={['table', 'indexLabel']} />}
    </Accordion.Section>
  )
  ...
  return (
    <>
      <Accordion>
        {panelGeneral}
        {panelDataSeries}
        {config.visualizationType === 'Box Plot' && panelMeasures}
        {hasRightAxis && config.series && config.visualizationType === 'Combo' && panelAssignSeriesAxis}
        {panelLeftValueAxisOrFormat}
        {hasRightAxis && panelRightValueAxis}
