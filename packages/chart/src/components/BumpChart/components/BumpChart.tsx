...
const BumpChart: React.FC<BumpChartProps> = ({ width = 700, height = 900, config, xScale, yScale }) => {
  ...
  const handleClick = (seriesItem: string) => {
    setActiveState(seriesItem)
    console.log(activeState)
    console.log(colorScale(activeState))
  }
          ...
          return (
            <>
              ...
              <LinePath
                ...
                stroke={(seriesItem === activeState || activeState === '') ? colorScale(seriesItem) : '#000000'}
                onClick={() => handleClick(seriesItem)}
                onMouseLeave={() => clearClick()}
                strokeWidth={5}
                defined={(item, i) => {
