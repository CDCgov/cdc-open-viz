...
const Legend = () => {
  ...
  if (!legend.dynamicLegend)
    return (
      <aside style={{ marginTop: config.legend.position === 'bottom' ? `${config.runtime.xAxis.size}px` : '0px', marginBottom: config.legend.position === 'bottom' ? '15px' : '0px' }} id='legend' className={containerClasses.join(' ')} role='region' aria-label='legend' tabIndex={0}>
