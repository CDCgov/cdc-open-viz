...
const Legend = () => {
  ...
  if (!legend.dynamicLegend)
    return (
      <aside style={{ marginTop: config.legend.position === 'bottom' && config.orientation === 'horizontal' ? `${config.runtime.xAxis.size}px` : '0px' }} id='legend' className={containerClasses.join(' ')} role='region' aria-label='legend' tabIndex={0}>
