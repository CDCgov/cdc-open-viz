...
export default function SparkLine({ width: parentWidth, height: parentHeight }) {
  const { transformedData: data, dimensions, config, parseDate, formatDate, currentViewport, seriesHighlight, formatNumber, colorScale, isNumber, handleChartAriaLabels } = useContext(Context)
  let width = parentWidth
  const { minValue, maxValue } = useReduceData(config, data, Context)
