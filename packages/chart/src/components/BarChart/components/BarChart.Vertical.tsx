            ...
            return barGroups.map((barGroup, _index) => (
              ...
              >
                {barGroup.bars.map((bar, index) => {
                    ...
                    if (dashboardConfig && dashboardConfig.dashboard.sharedFilters?.length !== 0) {
                      ...
                      if (isTwoToneLollipopColor) {
                        ...
                        if (lightness > 0.7) {
                          ...
                          _barColor = chroma(barColor).darken(0.1).hex()
                        } else if (lightness > 0.4) {
                          // Medium colors: lighten moderately
                          _barColor = chroma(barColor).brighten(0.5).hex()
                        } else {
                          // Dark colors: lighten significantly but keep visible
                          _barColor = chroma(barColor).brighten(1.0).hex()
                        }
                      }
                      return _barColor
                    }
                  ...
                  const getLollipopHeadColor = (barColor: string): string => {
                    if (!isTwoToneLollipopColor) {
                      ...
                      <Group key={`bar-sub-group-${barGroup.index}-${barGroup.x0}-${barY}--${index}`}>
                        ...
                        {config.isLollipopChart && config.lollipopShape === 'circle' && (
                          <circle
                            ...
                            fill={getLollipopHeadColor(colorScale(config.runtime.seriesLabels[bar.key]))}
