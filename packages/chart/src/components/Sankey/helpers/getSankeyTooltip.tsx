...
export const getSankeyTooltip = (data: Object, tooltipID: string) => {
  const tooltips = Array.isArray(data?.tooltips) ? data.tooltips : []
  const tooltipVal = `${(tooltips.find(item => item.node === tooltipID) || {}).value}`
  const tooltipSummary = `${(tooltips.find(item => item.node === tooltipID) || {}).summary}`
  const tooltipColumn1Label = (tooltips.find(item => item.node === tooltipID) || {}).column1Label
  const tooltipColumn2Label = (tooltips.find(item => item.node === tooltipID) || {}).column2Label
  const tooltipColumn1 = (tooltips.find(item => item.node === tooltipID) || {}).column1
  const tooltipColumn2 = (tooltips.find(item => item.node === tooltipID) || {}).column2
