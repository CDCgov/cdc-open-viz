            ...
            {props => {
              ...
              return (
                <Group className='left-axis'>
                  {props.ticks.map((tick, i) => {
                    return (
                      <Group key={`vx-tick-${tick.value}-${i}`} className={'vx-axis-tick'}>
                        {!config.runtime.yAxis.hideTicks && <Line from={tick.from} to={tick.to} stroke='#333' display={config.runtime.horizontal ? 'none' : 'block'} />}
                        ...
                        {config.orientation !== 'horizontal' && config.visualizationType !== 'Paired Bar' && !config.yAxis.hideLabel && (
                          <Text x={config.runtime.horizontal ? tick.from.x + 2 : tick.to.x} y={tick.to.y + (config.runtime.horizontal ? horizontalTickOffset : 0)} verticalAnchor={config.runtime.horizontal ? 'start' : 'middle'} textAnchor={config.runtime.horizontal ? 'start' : 'end'}>
                            {tick.formattedValue}
                          </Text>
                        )}
                      </Group>
                  ...
                  <Text className='y-label' textAnchor='middle' verticalAnchor='start' transform={`translate(${-1 * config.runtime.yAxis.size}, ${axisCenter}) rotate(-90)`} fontWeight='bold'>
                    {props.label}
                  </Text>
                </Group>
              )
              ...
              return (
                <Group className='bottom-axis'>
                  {props.ticks.map((tick, i) => {
                    ...
                    return (
                      <Group key={`vx-tick-${tick.value}-${i}`} className={'vx-axis-tick'}>
                        {!config.xAxis.hideTicks && <Line from={tick.from} to={tick.to} stroke='#333' />}
                  ...
                  <Text x={axisCenter} y={config.runtime.xAxis.size} textAnchor='middle' verticalAnchor='end' fontWeight='bold'>
                    {props.label}
                  </Text>
                </Group>
              )
