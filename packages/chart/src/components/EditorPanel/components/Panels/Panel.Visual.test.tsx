...
describe('PanelVisual series color assignments', () => {
  ...
  it('shows and updates the V2 distribution control for eligible charts', () => {
    const { updateConfig } = renderPanel(
      buildConfig({
        general: {
          palette: {
    ...
    const checkbox = screen.getByLabelText('Use V2 Color Distribution')
    ...
    expect(updateConfig).toHaveBeenCalledWith(
      expect.objectContaining({
        general: expect.objectContaining({
          palette: expect.objectContaining({ distributionVersion: '2.0' })
        })
      })
    )
  })
  ...
  it.each([
    ['ordinary chart', { general: { palette: { name: 'sequential_blue', version: '2.0' } } }],
    [
      'Pie chart',
      {
        visualizationType: 'Pie',
        general: { palette: { name: 'sequential_blue', version: '2.0' } }
      }
    ],
    [
      ...
      {
        visualizationType: 'Bar',
        legend: { colorCode: 'category' },
        general: { palette: { name: 'sequential_blue', version: '2.0' } }
      }
    ],
    ['divergent chart', { general: { palette: { name: 'divergent_blue_orange', version: '2.0' } } }],
    [
      'reversed sequential chart',
      {
        general: {
          palette: { name: 'sequential_bluereverse', version: '2.0', isReversed: true }
        }
      }
    ],
    [
      'color-coded Bar chart with a qualitative palette',
      {
        visualizationType: 'Bar',
        legend: { colorCode: 'category' },
  ...
  ])('shows the distribution control for a supported %s', (_name, overrides) => {
    renderPanel(buildConfig(overrides as Partial<ChartConfig>))

    expect(screen.getByLabelText('Use V2 Color Distribution')).toBeInTheDocument()
  })

  it('treats empty custom color arrays as inactive for the distribution control', () => {
    renderPanel(
      buildConfig({
        general: {
          palette: {
    ...
    expect(screen.getByLabelText('Use V2 Color Distribution')).toBeChecked()
  })

  it.each([
    [
      'custom colors',
      { general: { palette: { name: 'qualitative_standard', version: '2.0', customColors: ['#fff'] } } }
    ],
    [
      'ordered custom colors',
      {
        general: {
          palette: { name: 'qualitative_standard', version: '2.0', customColorsOrdered: ['#fff'] }
        }
      }
    ],
    ['V1 palette', { general: { palette: { name: 'qualitative_bold', version: '1.0' } } }],
    [
      'Horizon',
      { visualizationType: 'Horizon Chart', general: { palette: { name: 'qualitative_standard', version: '2.0' } } }
    ],
    [
      'unsupported chart',
      { visualizationType: 'Forest Plot', general: { palette: { name: 'qualitative_standard', version: '2.0' } } }
    ]
  ])('hides the distribution control for %s', (_name, overrides) => {
    renderPanel(buildConfig(overrides as Partial<ChartConfig>))

    expect(screen.queryByLabelText('Use V2 Color Distribution')).not.toBeInTheDocument()
  })
})
