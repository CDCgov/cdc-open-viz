...
const PanelGeneral: FC<PanelProps> = props => {
  ...
  const { enabledChartTypes, showBarStyleOptions, updateField } = useContext<EPContext>(EditorPanelContext)
  const { visHasNumbersOnBars, visHasLabelOnData, visSupportsChartHeight, visSupportsSuperTitle, visSupportsFootnotes } = useEditorPermissions()
  ...
  return (
    <AccordionItem>
      ...
      <AccordionItemPanel>
        ...
        {(visualizationType === 'Bar' || visualizationType === 'Deviation Bar') && <Select value={config.isLollipopChart ? 'lollipop' : config.barStyle || 'flat'} fieldName='barStyle' label='bar style' updateField={updateField} options={showBarStyleOptions()} />}
