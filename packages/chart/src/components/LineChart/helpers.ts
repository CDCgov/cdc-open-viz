...
export const createStyles = (props: StyleProps): Style[] => {
  ...
  let styles: Style[] = []
  const createStyle = (lineStyle, isDashed): Style => ({
    stroke: stroke,
    strokeWidth: 2,
  ...
  rawData.forEach((d, index) => {
    ...
    let style: Style = matchingPd ? createStyle(handleLineType(matchingPd.style), true) : createStyle(handleLineType(lineType), false)

    styles.push(style)

    // If matchingPd exists, update the previous style if there is a previous element
    if (matchingPd && index > 0) {
      styles[index - 1] = createStyle(handleLineType(matchingPd.style), true)
    }
  })
