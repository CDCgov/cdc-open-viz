...
function reducer<T>(state: State, action: Action<T>): State {     // <T> refers to generic type
  ...
  switch (action.type) {
    ...
    case SEQUENTIAL:
      ...
      return {
        ...state,
        filteredPallets: sequential,
        filteredQualitative: qualitative,
    ...
    case SEQUENTIAL_REVERSE:
      ...
      return {
        ...state,
        filteredQualitative: qualitativeReverse,
        filteredPallets: sequentialReverse,
        paletteName: paletteName,
...
function useColorPalette<T, Y extends Keyable>(colorPalettes: T, configState: Y) {
