...
| `general.showAnnotationsOnMobile` | `boolean` | No | `false` | Renders full annotation labels on mobile instead of the numbered-circle treatment. | When `false` (default), mobile shows numbered circles on the chart and auto-forces the annotation dropdown below the chart. When `true`, mobile renders the same HTML labels as desktop and the dropdown is no longer auto-forced (it follows `general.showAnnotationDropdown`). Author opt-in: labels may overflow narrow viewports. |
