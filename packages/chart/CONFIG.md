...
| `yAxis.autoMaxRounding` | `none \| tick-friendly` | No | `none` when omitted; new chart configs default to `tick-friendly` | Optionally rounds automatic value-axis maximums before padding. | Applies only when the relevant value-axis max is not explicitly set (`yAxis.max` for vertical charts, `xAxis.max` for horizontal charts, `yAxis.rightMax` for Combo right axes). `tick-friendly` rounds values above 5 up to a cleaner mantissa step such as `6`, `8`, `25`, `150`, `1500`, or `2500`; top-title auto-padding does not add another tick when that rounded top tick is already greater than or equal to the data maximum. Smaller values rely on `yAxis.smallestLeftAxisMax` or `yAxis.smallestRightAxisMax` when an author wants a floor. |
