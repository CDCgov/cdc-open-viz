...
| `yAxis` | [`Axis`](https://github.com/CDCgov/cdc-open-viz/blob/main/packages/core/CONFIG.md#axis) | Conditionally | Shared core default | Y-axis / measure-axis configuration. | Required for `Pie` charts and shared with multi-axis chart flows. See `@cdc/core` for nested fields. In charts, `yAxis.titlePlacement` controls whether the title stays on the side or moves above the plot. Top titles also keep the chart's y-axis auto-padding behavior enabled, including in small multiples. |
