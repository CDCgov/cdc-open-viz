...
| `dataFormat.prefix`, `dataFormat.suffix` | `string` | No | `''` | Text added before or after left/value-axis series numbers. | Examples: `$`, `%`. Series-column formatting may override these fields; blank series-column affixes inherit them. Additional tooltip/table columns use their own `columns.*.prefix` and `columns.*.suffix`; blank additional-column affixes do not inherit these global value-axis affixes. |
