...
| `yAxis.filterDomainBehavior` | `dynamic \| stable` | No | `dynamic` when omitted | Controls which rows automatic Y-axis domains use when filters are active. | `dynamic` rescales to the currently filtered rows. `stable` uses the full eligible chart dataset while rendering filtered rows; it still respects hard exclusions and chart dataset scope. Brush behavior remains controlled by `xAxis.brushDynamicYAxis`. |
| `yAxis.autoMaxRounding` | `none \| nice-power-of-ten` | No | `none` when omitted; new chart configs default to `nice-power-of-ten` | Optionally rounds automatic left Y-axis maximums before padding. | Applies only when `yAxis.max` is not explicitly set. `nice-power-of-ten` rounds values above 5 up to a clean power-of-ten step; smaller values rely on `yAxis.smallestLeftAxisMax` when an author wants a floor. Right-axis rounding is not supported. |
...
| `yAxis.inlineLabel`, `yAxis.labelPlacement`, `yAxis.labelsAboveGridlines` | `string`, `string`, `boolean` | No | None, package defaults, `false` | Inline suffix/label controls for the left Y axis. | Used by line and bar axis-label rendering and auto-padding helpers. |
