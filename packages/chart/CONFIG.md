...
| `yAxis.autoMaxRounding` | `none \| nice-power-of-ten` | No | `none` when omitted; new chart configs default to `nice-power-of-ten` | Optionally rounds automatic value-axis maximums before padding. | Applies only when the relevant value-axis max is not explicitly set (`yAxis.max` for vertical charts, `xAxis.max` for horizontal charts, `yAxis.rightMax` for Combo right axes). `nice-power-of-ten` rounds values above 5 up to a clean power-of-ten step; smaller values rely on `yAxis.smallestLeftAxisMax` or `yAxis.smallestRightAxisMax` when an author wants a floor. |
