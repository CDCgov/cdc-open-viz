...
| `series` | [`Series`](https://github.com/CDCgov/cdc-open-viz/blob/main/packages/core/CONFIG.md#series) | Conditionally | Shared core default | Measure series to render. | Required for non-pie charts unless `dynamicSeries` is enabled. For `HeatMap`, each selected data series becomes one rendered row. |
...
| `heatmap` | `object` | No | `{ cellPadding: 1, rowLabelGap: 32, columnLabelGap: 56, xAxisPosition: 'top', showCellValues: false }` | HeatMap-specific layout and cell display settings. | `HeatMap` uses `xAxis.dataKey` for columns and `series` for rows; each selected series produces one heatmap row. |
...
| `heatmap.xAxisPosition` | `'top' \| 'bottom'` | No | `'top'` | Places the HeatMap x-axis above or below the cell grid. | `top`, `bottom`. |
| `heatmap.showCellValues` | `boolean` | No | `false` | Shows formatted numeric values inside HeatMap cells. | Text color is selected from dark or white based on the cell fill color for contrast. Values are hidden for very small cells to avoid overlap. |
